...
# With metadata, pageless PDF, and publish to GitHub Pages:
node generate.js "/path/to/videos" --title "Pretty Title" \
  --client "Client Name" --url "https://frame.io/..." --notes "First pass" \
  --pdf --pageless --publish client-slug
```

Published pages land at `https://c7sharp9.github.io/client-preview/<slug>.html`. A README in the `client-preview` repo (auto-generated by GitHub Action) lists every published page with its full live URL.
...
- `make-sheet` interactive wrapper prompts for folder, title, client, video files URL, notes, and publish slug. Client auto-fills from last run; slug auto-fills from title.
- Project metadata (client, video files URL, notes, contact info) renders above the thumbnail grid. Contact defaults to Jonathan Ayala / jon@innerviewmedia.com.
- `client-preview` README auto-generated by GitHub Action with full live URLs on every push. No public index page (clients only see pages shared via direct URL).
- `--publish` pulls before pushing to stay in sync with GitHub Action commits.
